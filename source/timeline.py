...
class Timeline:
    """
    The Lookup class manages the connection to the twitter tweets  API through the
    ``GET /2/tweets`` request. The poll class allows one to query reaction counts (retweets, replies, quotes, likes)
    for a set of tweet ids.
    """
    ...
    def pull(self, output_dir: str):
        ...
        """
        print("Beginning timeline pull.")

        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # iterate over members, retrieving tweet responses
        for ix, handle in enumerate(self.handles[:3]):
            print(f"Polling tweets for user handle {handle}.")

            # attempt to get user_id
            try:
                user_id = self.client.get_user(username=handle).data.id
            except Exception as e:
                print("Failed to get user id for {handle}. Continuing to next handle.")
                continue
            ...
            while not finished:
                ...
                if next_token is None:
                    finished = True
                    print(f"Collected {num_collected} new tweets for user id {handle}")
